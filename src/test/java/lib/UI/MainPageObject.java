...
//    }
    ...
    public void getWordInSearchResult(By by, String value) {
        List<WebElement> articles = driver.findElements(by);
        for (WebElement temp : articles) {
            String txt = temp.getAttribute("text");
            boolean isContain = txt.toLowerCase().contains(value.toLowerCase());
            Assert.assertEquals(
                    "No needed word",
                    true,
                    isContain);
    ...
    public void assertElementPresent(String locator, String error_message) {
        int title_of_article = this.getAmountOfElements(locator);
        if (title_of_article == 0) {
